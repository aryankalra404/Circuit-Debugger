...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    socket.on('circuit:update', (payload: { sessionId?: string; circuit?: CircuitSnapshot } | CircuitSnapshot) => {
      const nextCircuit = (payload as { circuit?: CircuitSnapshot }).circuit || payload as CircuitSnapshot;
      if (nextCircuit) setCircuit(nextCircuit);
    });
  ...
  return <main className="min-h-screen px-5 py-8 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl">
    <header className="mb-8 flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">CircuitDoctor</p><h1 className="text-2xl font-semibold tracking-tight text-slate-950">Live circuit diagnosis</h1><p className="mt-2 text-sm text-slate-500">A live structural mirror of the circuit being built in Quest.</p></div>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-panel sm:p-7">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Live AR circuit status</p><h2 className="mt-2 text-lg font-semibold tracking-tight text-slate-950">Quest circuit graph</h2><p className="mt-1 text-sm text-slate-500">Nodes and wires appear as the structural circuit update arrives.</p></div><div className="flex items-center gap-2 text-xs font-medium text-slate-500"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: circuitStatusColors.unconnected.stroke }} /> Unconnected <span className="ml-2 h-2 w-2 rounded-full" style={{ backgroundColor: circuitStatusColors.connected.stroke }} /> Connected</div></div>
      <CircuitGraph circuit={circuit} faultComponents={faultComponents} confidence={result.confidence || null} />
      <div className={`mt-6 rounded-xl border p-5 ${palette.tailwind}`}><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border-4 border-white shadow-sm" style={{ backgroundColor: palette.stroke }}><span className="text-base font-bold text-white">{isOk ? '✓' : hasResult ? '!' : '…'}</span></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold uppercase tracking-[0.16em] opacity-70">{isOk ? 'Circuit OK' : hasResult ? 'Circuit fault' : 'No reading yet'}</p>{hasResult && result.confidence && <span className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold capitalize leading-4 ${badgeClassForConfidence(result.confidence)}`}>{result.confidence}</span>}</div><p className="mt-1 text-base font-medium leading-6">{result.message}</p>{hasResult && result.confidence && result.groundedOn && <p className="mt-2 text-xs leading-5 text-slate-500">Source: {result.groundedOn}</p>}{hasResult && faultComponents.length > 0 && <p className="mt-2 text-xs font-medium text-slate-600">Affected: {faultComponents.join(', ')}</p>}</div></div></div>
      <div className="mt-6 grid grid-cols-2 gap-3 border-t border-slate-100 pt-6 text-sm"><div><p className="text-xs text-slate-500">Bridge</p><p className="mt-1 font-medium text-slate-800">{connected ? 'Online' : 'Offline'}</p></div><div><p className="text-xs text-slate-500">Components</p><p className="mt-1 font-medium text-slate-800">{circuit?.components?.length || 0}</p></div></div>
    </section>
  </div></main>;
}
