...
export const circuitStatusColors = {
  ...
  waiting: { fill: '#f8fafc', stroke: '#94a3b8', text: '#475569', tailwind: 'border-slate-200 bg-slate-50 text-slate-600' }
} as const;

export function paletteForResult(ok: boolean, confidence: Confidence, hasResult = true) {
  if (!hasResult) return circuitStatusColors.waiting;
